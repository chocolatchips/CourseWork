...
{
	bool success;
	std::string pathName = "objectpath";
	std::string partSysName = "partSys";
	
	ParticleSystem* partSys = new ParticleSystem(partSysName);
	success = GlobalResourceManager::use()->addSystem(partSys, true);
	assert(success);

	std::string partSimName = "partSim";
	ParticleSimulator* splineSimulator = new ParticleSimulator(partSimName, partSys);
	success = GlobalResourceManager::use()->addSimulator(splineSimulator);
	assert(success);

	BaseSystem* sampleSystemRetrieval;
	sampleSystemRetrieval = GlobalResourceManager::use()->getSystem(partSysName);
	assert(sampleSystemRetrieval);

	BaseSimulator* sampleSimulatorRetrieval;
	sampleSimulatorRetrieval = GlobalResourceManager::use()->getSimulator(partSimName);
...
static int testLinkGlobalCommand(ClientData clientData, Tcl_Interp* interp, int argc, myCONST_SPEC char** argv)
{
	animTcl::OutputMessage("This is a test link command!");
	animTcl::OutputResult("100");
	return TCL_OK;
